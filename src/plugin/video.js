...
const video = async (m, Matrix) => {
  ...
  const validCommands = ['video', 'ytmp4', 'vid'];
   ...
   if (validCommands.includes(cmd)) {
    if (!text) return m.reply('Give a YouTube URL or search query Mercedes.');
      ...
      if (isUrl) {
        ...
        videoStream.on('end', async () => {
          try {
            ...
            await Matrix.sendMessage(m.from, { video: finalVideoBuffer, mimetype: 'video/mp4', caption: '> © Powered by Mercedes' }, { quoted: mek });
            await m.React("✅");
          } catch (err) {
            console.error('Error sending video:', err);
            m.reply('Error sending video.');
